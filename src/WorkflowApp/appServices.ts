...
class AppServices {
  ...
  async applyPaymentRef(input, context) {
    await delay(5, "5s"); // Wait for 2.1 mins
    context.item.data.whathasbeendone = "applyPaymentRef";
    console.log("   ****** Called applyPaymentRef");
  }

  async DummyService2(input, context) {
    await delay(60, "60"); // Wait for 2.1 mins
